...
def main():
    ...
    print('Generating traffic for BLUE flow (h2-h8)')
    rate = int(math.ceil(50e6/8.0/(msglen)))
    pkts = []
    for i in range(rate*seconds):
        beg = random.randint(0, 1e6 - msglen - 1)
        pkt = Ether() / IP(src='10.0.1.2', dst='10.0.4.8') / \
            UDP(sport=1234, dport=1234) / lorem[beg:beg+msglen]
        pkt.time = (1.0/rate)*i
        pkts.append(pkt)
        if i%100 == 0:
    ...
    print('Generating traffic for TEAL flow (h3-h7)')
    rate = int(math.ceil(50e6/8.0/(msglen)))
    pkts = []
    for i in range(rate*seconds):
        beg = random.randint(0, 1e6 - msglen - 1)
        pkt = Ether() / IP(src='10.0.2.3', dst='10.0.4.7') / \
            UDP(sport=1234, dport=1234) / lorem[beg:beg+msglen]
        pkt.time = (1.0/rate)*i
        pkts.append(pkt)
        if i%100 == 0:
    ...
    print('Generating traffic for GREEN flow (h4-h7)')
    rate = int(math.ceil(50e6/8.0/(msglen)))
    pkts = []
    for i in range(rate*seconds):
        beg = random.randint(0, 1e6 - msglen - 1)
    ...
    print('Generating traffic for ORANGE flow (h4-h9)')
    rate = int(math.ceil(50e6/8.0/(msglen)))
    pkts = []
    for i in range(rate*seconds):
        beg = random.randint(0, 1e6 - msglen - 1)
        pkt = Ether() / IP(src='10.0.2.4', dst='10.0.5.9') / \
            UDP(sport=1234, dport=1234) / lorem[beg:beg+msglen]
        pkt.time = (1.0/rate)*i
        pkts.append(pkt)
        if i%100 == 0:
